...
class tcl_main definition inheriting from zcl_abap_sync
...
class tcl_other_cx definition inheriting from zcl_promise_resolver.
  public section.
    methods then redefinition.
endclass.
...
class tcl_main implementation.
  method test_await_resolved.
    ...
    try.
        ...
        data(lr_result) = await( new tcl_resolved( ) ).
        assign lr_result->* to field-symbol(<lv_result>).
  ...
  method test_await_rejected.
    ...
    try.
        ...
        assign lo_cx->with->* to field-symbol(<lv_result>).
